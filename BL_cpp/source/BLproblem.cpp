...
double BLproblem::k_rw(double sw_cur) {
    auto ans = std::pow(sw_cur, nw[0]) * kw0;
    return ans;

};

double BLproblem::k_ro(double sw_cur) {
    auto ans = std::pow(1. - sw_cur, no[0]) * ko0;
    return ans;
};

double BLproblem::pc(double &sw_cur) {
    return pow(sw_cur, m[0]);
};

double BLproblem::sigma(double &sw_cur) {
    return sw_cur * phi;
};

double BLproblem::f(double &sw_cur) {
    return (k_rw(sw_cur) / mu_w) / (k_rw(sw_cur) / mu_w + k_ro(sw_cur) / mu_o);
};

double BLproblem::g(double &sw_cur) {
    return f(sw_cur) * u;
};
...
BLproblem::solver(double t0, double tn, double time_steps, double sw0, double swn) {
    ...
    while (t <= tn) {
        ...
        for (int i = 1; i < sw.size() - 1; ++i) {
            ...
            file << "sw_prev:  " << sw_prev[i] << " " << sw_prev[i - 1] << std::endl;
            file << "nw:  " << nw[i] << " no: " << no[i - 1] << std::endl;
            file << "k_rw(sw = " << sw[i] << ", i = " << i << ") = " << k_rw(sw[i]) << std::endl;
            file << "k_ro(sw = " << sw[i] << ", i = " << i << ") = " << k_ro(sw[i]) << std::endl;
